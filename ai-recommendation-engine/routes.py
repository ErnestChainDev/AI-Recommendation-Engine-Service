...
def build_router(SessionLocal):
    ...
    async def recommend(payload: RecommendIn, db: Session = Depends(get_db)):
        ...
        # Merge profile skills into interests text for feature vector
        if skills:
        ...
        #    Ensure user_skills / user_interests / user_career_goals
        #    are always List[str] regardless of what the client sends.
        # ----------------------------
        ...
        result = recommend_with_kmeans_and_cbf(
            ...
            # ✅ FIX: pass structured profile lists so scoring uses them
            user_skills       = payload_skills,
            user_interests    = payload_interests,
            user_career_goals = payload_career_goals,
        ...
        upsert_recommendation_result(
            ...
            user_id           = payload.user_id,
            attempt_id        = payload.attempt_id,
            program           = result["recommended_program"],   # ✅ correct key
            confidence        = result["confidence"],
            message           = result["message"],
            percent_score     = result["percent_score"],
            gwa               = result["gwa"],
        ...
        # Attach derived fields before returning
